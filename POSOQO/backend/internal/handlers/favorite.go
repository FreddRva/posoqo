...
func AddFavorite(c *fiber.Ctx) error {
	...
	fmt.Printf("🔍 [BACKEND] AddFavorite - userID: %d, productID: %s\n", userID, requestBody.ProductID)
	
	_, err := db.DB.Exec(context.Background(),
		"INSERT INTO favorites (user_id, product_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",
		userID, requestBody.ProductID)
	if err != nil {
		fmt.Printf("🔍 [BACKEND] Error insertando favorito: %v\n", err)
		return c.Status(http.StatusInternalServerError).JSON(fiber.Map{"error": "No se pudo agregar a favoritos"})
	}
	
	fmt.Printf("🔍 [BACKEND] Favorito insertado exitosamente\n")
...
func ListFavorites(c *fiber.Ctx) error {
	...
	// Log para debug
	fmt.Printf("🔍 [BACKEND] ListFavorites - userID: %d\n", userID)
	...
	if err != nil {
		fmt.Printf("🔍 [BACKEND] Error en query: %v\n", err)
	...
	for rows.Next() {
		...
		count++
		fmt.Printf("🔍 [BACKEND] Producto encontrado: %s (ID: %s)\n", name, id)
	...
	fmt.Printf("🔍 [BACKEND] Total productos encontrados: %d\n", count)
